...
import MentalHealthTraumaDisorderPtsd from "@/components/templates/MentalHealthTraumaDisorderPtsd";
...
const PREFIX = "/mental-health/trauma-disorder/ptsd";
const FALLBACK_SLUGS = ["combat-ptsd","ptsd-counseling","complex-ptsd"] as string[];
...
export async function generateStaticParams() {
  ...
  try {
    const res = await fetch(
      `${wpUrl}/wp-json/builder/v1/pages?path=%2Fmental-health%2Ftrauma-disorder%2Fptsd`,
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const Template: ComponentType<Record<string, string>> = (await resolveTemplate(data)) ?? MentalHealthTraumaDisorderPtsd;
