...
import MentalHealthNeurodevelopmentDisorderAdhd from "@/components/templates/MentalHealthNeurodevelopmentDisorderAdhd";
...
const PREFIX = "/mental-health/neurodevelopment-disorder/adhd";
const FALLBACK_SLUGS = ["adhd-paralysis","is-adhd-neurodivergent","passive-suicidal-ideation-adhd"] as string[];
...
export async function generateStaticParams() {
  ...
  try {
    const res = await fetch(
      `${wpUrl}/wp-json/builder/v1/pages?path=%2Fmental-health%2Fneurodevelopment-disorder%2Fadhd`,
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const Template: ComponentType<Record<string, string>> = (await resolveTemplate(data)) ?? MentalHealthNeurodevelopmentDisorderAdhd;
