...
import "@/app/shared/pages/8c333665b6d5.css";
import type { Metadata } from "next";
import { notFound } from "next/navigation";
import type { ComponentType } from "react";
import MentalHealthNeurodevelopmentDisorder from "@/components/templates/MentalHealthNeurodevelopmentDisorder";
...
const PREFIX = "/mental-health/neurodevelopment-disorder";
const FALLBACK_SLUGS = ["dyspraxia","autism-spectrum-disorder"] as string[];
...
export async function generateStaticParams() {
  ...
  try {
    const res = await fetch(
      `${wpUrl}/wp-json/builder/v1/pages?path=%2Fmental-health%2Fneurodevelopment-disorder`,
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const Template: ComponentType<Record<string, string>> = (await resolveTemplate(data)) ?? MentalHealthNeurodevelopmentDisorder;
