...
import MentalHealthVs from "@/components/templates/MentalHealthVs";
...
const PREFIX = "/mental-health/vs";
const FALLBACK_SLUGS = ["add-vs-adhd","intrusive-vs-impulsive-thoughts","ocd-vs-ocpd","cptsd-vs-ptsd","borderline-personality-disorder-vs-bipolar","anxiety-attack-panic-attack"] as string[];
...
export async function generateStaticParams() {
  ...
  try {
    const res = await fetch(
      `${wpUrl}/wp-json/builder/v1/pages?path=%2Fmental-health%2Fvs`,
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const Template: ComponentType<Record<string, string>> = (await resolveTemplate(data)) ?? MentalHealthVs;
