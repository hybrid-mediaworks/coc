...
import "@/app/shared/pages/bf4e2b7c7d9f.css";
import type { Metadata } from "next";
import { notFound } from "next/navigation";
import type { ComponentType } from "react";
import MentalHealthSleepDisorder from "@/components/templates/MentalHealthSleepDisorder";
...
const PREFIX = "/mental-health/sleep-disorder";
const FALLBACK_SLUGS = ["night-terrors","alcohol-and-insomnia"] as string[];
...
export async function generateStaticParams() {
  ...
  try {
    const res = await fetch(
      `${wpUrl}/wp-json/builder/v1/pages?path=%2Fmental-health%2Fsleep-disorder`,
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const Template: ComponentType<Record<string, string>> = (await resolveTemplate(data)) ?? MentalHealthSleepDisorder;
