...
import "@/app/shared/pages/507a0dc76ef5.css";
import type { Metadata } from "next";
import { notFound } from "next/navigation";
import type { ComponentType } from "react";
import MentalHealthPsychoticDisordersSchizophrenia from "@/components/templates/MentalHealthPsychoticDisordersSchizophrenia";
...
const PREFIX = "/mental-health/psychotic-disorders/schizophrenia";
const FALLBACK_SLUGS = ["schizoaffective-disorder","paranoid-schizophrenia","schizophrenia-support-groups","is-schizophrenia-genetic","types-of-schizophrenia"] as string[];
...
export async function generateStaticParams() {
  ...
  try {
    const res = await fetch(
      `${wpUrl}/wp-json/builder/v1/pages?path=%2Fmental-health%2Fpsychotic-disorders%2Fschizophrenia`,
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const Template: ComponentType<Record<string, string>> = (await resolveTemplate(data)) ?? MentalHealthPsychoticDisordersSchizophrenia;
