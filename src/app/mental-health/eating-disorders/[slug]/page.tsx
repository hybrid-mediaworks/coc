...
import MentalHealthEatingDisorders from "@/components/templates/MentalHealthEatingDisorders";
...
const PREFIX = "/mental-health/eating-disorders";
const FALLBACK_SLUGS = ["body-dysmorphic-disorder","what-is-bulimia","do-i-have-an-eating-disorder","are-eating-disorders-genetic"] as string[];
...
export async function generateStaticParams() {
  ...
  try {
    const res = await fetch(
      `${wpUrl}/wp-json/builder/v1/pages?path=%2Fmental-health%2Feating-disorders`,
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const Template: ComponentType<Record<string, string>> = (await resolveTemplate(data)) ?? MentalHealthEatingDisorders;
