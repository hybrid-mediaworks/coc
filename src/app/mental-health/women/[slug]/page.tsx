...
import MentalHealthWomen from "@/components/templates/MentalHealthWomen";
...
const PREFIX = "/mental-health/women";
const FALLBACK_SLUGS = ["mom-burnout","womens-mental-health"] as string[];
...
export async function generateStaticParams() {
  ...
  try {
    const res = await fetch(
      `${wpUrl}/wp-json/builder/v1/pages?path=%2Fmental-health%2Fwomen`,
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const Template: ComponentType<Record<string, string>> = (await resolveTemplate(data)) ?? MentalHealthWomen;
