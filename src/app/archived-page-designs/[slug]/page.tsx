...
import ArchivedPageDesigns from "@/components/templates/ArchivedPageDesigns";
...
const PREFIX = "/archived-page-designs";
const FALLBACK_SLUGS = ["home-v3","elementor-4434","home-v2","home-v3-copy"] as string[];
...
export async function generateStaticParams() {
  ...
  try {
    const res = await fetch(
      `${wpUrl}/wp-json/builder/v1/pages?path=%2Farchived-page-designs`,
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const Template: ComponentType<Record<string, string>> = (await resolveTemplate(data)) ?? ArchivedPageDesigns;
