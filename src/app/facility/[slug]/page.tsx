...
import "@/app/shared/pages/78ef5253c23a.css";
import type { Metadata } from "next";
import { notFound } from "next/navigation";
import type { ComponentType } from "react";
import Facility from "@/components/templates/Facility";
...
const FALLBACK_SLUGS = ["connections-mental-health-treatment-center-orange-county-ca","connections-oc-mental-health-treatment-center-tustin-ca"] as string[];
...
export async function generateStaticParams() {
  ...
  try {
    const res = await fetch(
      `${wpUrl}/wp-json/builder/v1/pages?path=%2Ffacility`,
...
export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!data) notFound();
  const Template: ComponentType<Record<string, string>> = Facility;
