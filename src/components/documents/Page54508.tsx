import React from 'react';
import RichText from '@/components/RichText';
import { TrustIndexWidget } from '@/components/TrustIndexWidget';
import RelatedPages from '@/components/RelatedPages';
import Image from 'next/image';
import Link from 'next/link';
import BlogTocList from '@/components/BlogTocList';


export default function Page54508(props: Record<string, string>) {
  const __presentIds = (props.__present ?? "").split(",").filter(Boolean);
  const __present = (id: string) => __presentIds.length === 0 || __presentIds.includes(id);
  const blog_section_1___con = props.blog_section_1___con ?? "<h2>What is Borderline Personality Disorder?</h2>\r\n<img src=\"https://connectionsoc.com/wp-content/uploads/2023/09/pexels-timur-weber-8560216-scaled.jpg\" alt=\"a person in therapy representing what is dynamic interpersonal therapy\" />\r\n\r\nBorderline personality disorder involves dramatic mood swings, volatile interpersonal relationships, and impulsivity. Those with BPD often experience a profound fear of abandonment, struggle with anger management, and engage in impulsive and risky actions like reckless driving and self-harm threats, affecting their ability to maintain relationships.\r\n\r\nOne of four cluster B personality disorders, BPD is associated with erratic and dramatic behaviors that lead to chronic dysfunctional behavior patterns, causing distress and social difficulties. Many individuals with BPD remain unaware of their condition and unaware that healthier behavioral and relational alternatives exist.\r\n<h2>Borderline Personality Disorder Symptoms</h2>\r\nThe symptoms of borderline personality disorder are categorized into nine major aspects for diagnostic purposes. To receive a BPD diagnosis, at least five borderline personality disorder criteria must present, impacting multiple aspects of life:\r\n\r\n[accordion_block]\r\n<h3>Fear of abandonment</h3>\r\nIndividuals with BPD often fear abandonment, leading to desperate efforts to prevent it, which can inadvertently push people away.\r\n<h3>Unstable relationships</h3>\r\nBPD leads to intense but short-lived relationships with rapid shifts between idealization and devaluation.\r\n<h3>Shifting self-image</h3>\r\nBPD results in an unstable self-concept, causing changes in jobs, relationships, values, and identity.\r\n<h3>Impulsive, self-destructive behaviors</h3>\r\nBPD individuals may engage in harmful actions like reckless driving or substance abuse as impulsive responses to distress.\r\n<h3>Self-harm</h3>\r\nSigns of borderline personality disorder may include suicidal tendencies, gestures, threats, or non-suicidal self-harm like cutting.\r\n<h3>Extreme emotional swings</h3>\r\nBPD involves rapid and intense mood shifts, which are relatively short-lived compared to episodes of depression or bipolar disorder.\r\n<h3>Chronic feelings of emptiness</h3>\r\nIndividuals with BPD often feel a persistent inner void, attempting to fill it with substances or behaviors.\r\n<h3>Explosive anger</h3>\r\nBPD can lead to intense anger, with difficulty in anger management, sometimes directed inwardly.\r\n<h3>Suspicion and dissociation</h3>\r\nBPD can cause paranoid thoughts and dissociation, leading to feeling detached from reality, especially during stress.\r\n\r\n[/accordion_block]\r\n<img src=\"https://connectionsoc.com/wp-content/uploads/2023/08/aiony-haust-f2ar0ltTvaI-unsplash-scaled.jpg\" alt=\"woman has hand on face representing Schizoaffective Disorder\" />\r\n<h2>Borderline Personality Disorder vs. Bipolar Disorder&lt;/h2</h2>\r\nAlthough bipolar disorder and borderline personality disorder share mood fluctuations, they are separate conditions.\r\n\r\nIn BPD, mood and behavior swiftly shift due to stress, especially in social interactions. In bipolar disorder, by contrast, mood changes are more prolonged and less responsive, accompanied by noticeable alterations in energy and activity levels that are not as prominent in BPD.\r\n\r\n[accordion_block]\r\n<h3>Persistent sadness</h3>\r\nIndividuals with depression often experience an overpowering feeling of sadness or emptiness. This emotional state may seem unshakable, making it challenging to find joy or happiness in things that once brought pleasure.\r\n<h3>Loss of interest or pleasure (anhedonia)</h3>\r\nOne of the hallmark symptoms of depression is anhedonia, where the person loses interest or pleasure in activities they used to enjoy. Hobbies, social interactions, or favorite pastimes may no longer hold the same appeal, contributing to a sense of disconnection from life.\r\n<h3>Changes in appetite or weight</h3>\r\nDepression can influence appetite, leading to significant changes in eating habits. Some individuals may experience increased cravings and overeats, resulting in weight gain, while others may have a reduced appetite and experience weight loss.\r\n<h3>Sleep disturbances</h3>\r\nInsomnia or hypersomnia (excessive sleep) are common sleep disturbances associated with depression. Insomnia may manifest as difficulty falling asleep, frequent awakenings during the night, or waking and being unable to fall back asleep.\r\n<h3>Fatigue and low energy</h3>\r\nFeeling persistently tired or experiencing a lack of energy, even after adequate rest, is a common symptom of depression. Tasks that were once manageable may become overwhelming and exhausting.\r\n<h3>Feelings of worthlessness or guilt</h3>\r\nDepression can lead to a profound sense of worthlessness or excessive guilt over perceived shortcomings or past mistakes. These feelings may be irrational and disproportionate to the circumstances.\r\n<h3>Difficulty concentrating and making decisions</h3>\r\nDepression can impair cognitive function, making it challenging to concentrate, focus, or make decisions. Memory may also be affected, making it harder to recall information or follow through with tasks.\r\n<h3>Physical aches and pains</h3>\r\nSome people with depression may experience physical symptoms such as headaches, stomachaches, muscle pain, or other unexplained bodily discomforts.\r\n<h3>Thoughts of death or suicide</h3>\r\nIn severe cases, individuals with depression may have recurrent thoughts of death or suicidal ideation. If you or someone you know is experiencing thoughts of suicide, seek immediate help from a mental health professional or a helpline.\r\n\r\n[/accordion_block]\r\n<h2>Borderline Personality Disorder Treatment</h2>\r\nHistorically, treating BPD has presented challenges, but modern evidence-based borderline personality disorder treatments are associated with:\r\n<ul>\r\n \t<li>Reduced symptoms</li>\r\n \t<li>Enhanced functionality</li>\r\n \t<li>Improved quality of life</li>\r\n</ul>\r\nSuccessful therapy for borderline personality disorder requires time, dedication, and patience, often involving psychotherapy, medications, or both.\r\n\r\nIn cases of extreme distress or self-harm risk, brief hospitalization might be indicated, enabling collaborative treatment planning. Co-occurring mental health issues are common among those with BPD, including:\r\n<ul>\r\n \t<li>Mood disorders</li>\r\n \t<li>Anxiety disorders</li>\r\n \t<li>Substance use disorders</li>\r\n \t<li>Eating disorders</li>\r\n \t<li>ADHD (attention deficit hyperactivity disorder)</li>\r\n \t<li>Bipolar disorder</li>\r\n \t<li>Sleeping disorders</li>\r\n</ul>\r\nTreating these conditions is equally important.\r\n\r\nPsychotherapy, informally known as talk therapy, is the primary treatment approach for BPD, aiming to explore underlying motivations and fears, as well as promoting positive interpersonal connections.\r\n<ul>\r\n \t<li>DBT (dialectical behavior therapy)</li>\r\n \t<li>CBT (cognitive behavioral therapy)</li>\r\n \t<li>Group therapy</li>\r\n \t<li>ST (schema therapy)</li>\r\n \t<li>MBT (mentalization-based therapy)</li>\r\n \t<li>TFP (transference-focused psychotherapy)</li>\r\n \t<li>Medications</li>\r\n</ul>\r\n<h2>DBT (dialectical behavior therapy)</h2>\r\nOriginally created to treat borderline personality disorder, DBT combines acceptance of reality and behavior change. It equips individuals with skills to manage emotions, reduce self-destructive behavior, and enhance relationships.\r\n<h2>CBT (cognitive behavioral therapy)</h2>\r\nStructured and goal-oriented, CBT focuses on analyzing thoughts and emotions, modifying negative patterns, and promoting healthier thinking and behavior.\r\n<h2>Group therapy</h2>\r\nConducted under professional guidance, group therapy aids BPD individuals in constructive interaction and effective self-expression.\r\n<h2>Schema therapy (ST)</h2>\r\nSchema therapy, conducted individually or in groups, aids in identifying unmet needs that have shaped detrimental life patterns originating from past survival mechanisms.\r\n<h2>MBT (mentalization-based therapy)</h2>\r\nMBT is a form of talk therapy that fosters self-awareness of thoughts and emotions, enabling a nuanced perspective on situations.\r\n<h2>TFP (transference-focused psychotherapy)</h2>\r\nTransference-focused psychotherapy is a psychodynamic approach that explores emotions and relationship challenges within therapy and applies insights to real life.\r\n<h2>Medications</h2>\r\nAlthough medications are not typically the mainstay for BPD treatment, they may help manage symptoms like anxiety, depression, mood swings, and impulsive behaviors.\r\n\r\n<img src=\"https://connectionsoc.com/wp-content/uploads/2023/08/pexels-shvets-production-7176027-150x150.jpg\" alt=\"therapy image\" />\r\n<h2>Borderline Personality Disorder FAQ</h2>\r\n[accordion_block]\r\n<h3>What is borderline personality disorder splitting?</h3>\r\nBorderline personality disorder splitting refers to a symptom where individuals with BPD tend to view things, including people, as all good or all bad, leading to intense shifts in emotions and relationships.\r\n<h3>How do I get tested for borderline personality disorder?</h3>\r\nThere is no specific borderline personality disorder test. Instead, mental health professionals conduct clinical assessment, considering patterns of behavior, emotions, and relationships, often over an extended period.\r\n<h3>What causes borderline personality disorder?</h3>\r\nThe exact cause of borderline personality disorder is not fully understood, but it is believed to result from a combination of genetic, neurological, and environmental factors, such as childhood trauma or disrupted attachments.\r\n<h3>Is borderline personality disorder treatable?</h3>\r\nYes, borderline personality disorder is treatable. Therapies like DBT (dialectical behavior therapy), CBT (cognitive-behavioral therapy), and medication can help manage symptoms and improve overall functioning, although results may vary from person to person\r\n\r\n[/accordion_block]\r\n<h2>Get Treatment for Borderline Personality Disorder at Connections</h2>\r\nBorderline personality disorder can be a disruptive condition, but with expert guidance and <a href=\"https://connectionsoc.com/mental-health-treatment/\">personalized mental health treatment</a>, it is possible to improve functioning and overall well-being. We can help you initiate this process at Connections Mental Health in Southern California.\r\n\r\nWe designed our beachside facility to be as welcoming as a home. Our committed team of highly experienced professionals is dedicated to delivering cutting-edge care that blends holistic and science-based treatments, with at least two staff members present at all times.\r\n\r\nWhether you or someone you know is grappling with a personality disorder, find stability, serenity, and support at Connections Mental Health. Call admissions today at <a href=\"tel:+18882552112\">888-255-2112</a> for more information and immediate assistance.\r\n\r\n<img src=\"https://connectionsoc.com/wp-content/uploads/2023/09/pexels-fauxels-3228726-scaled.jpg\" alt=\"two people hold hands representing boderline personality disorder treatment\" />";
  const h2___con = props.h2___con ?? "Borderline personality disorder (BPD) is a mental health condition that is characterized by intense mood fluctuations, unstable relationships, and difficulties in emotional regulation. Individuals with BPD face an increased risk of self-destructive behavior and suicide. The most effective intervention for BPD is talk therapy, sometimes supplemented with medications. Read on to discover how you can connect with treatment for borderline personality disorder – you do not need to suffer alone or in silence.";
  const h2___head = props.h2___head ?? "Borderline Personality Disorder (BPD): Signs, Causes, and Effective Treatment";
  const h1 = props.h1 ?? "Borderline Personality Disorder (BPD)";
  const brand = props.brand ?? "Connections Mental Health";
  const __hasData = (ff: string[]) => ff.some((k) => ((props[k] ?? "") + "").trim() !== "");
  return (
    <>
    <div className="wp-singular page-template-default page page-id-6081 page-parent page-child parent-pageid-6079 wp-embed-responsive wp-theme-hello-elementor facilit-gallery--empty hello-elementor-default elementor-default elementor-kit-6 elementor-page elementor-page-6081 elementor-page-54508">
  <svg id="asw-filters-svg" aria-hidden="true" style={{"display":"none"}}>
    <defs>
      <filter id="asw-protanopia-filter">
        <feColorMatrix type="matrix"></feColorMatrix>
      </filter>
      <filter id="asw-deuteranopia-filter">
        <feColorMatrix type="matrix"></feColorMatrix>
      </filter>
      <filter id="asw-tritanopia-filter">
        <feColorMatrix type="matrix"></feColorMatrix>
      </filter>
    </defs>
  </svg>
  <a className="skip-link screen-reader-text" href="#content" style={{"fontSize":"16px"}}>
    Skip to content
  </a>
  <div className="elementor elementor-54508 elementor-location-single post-6081 page type-page status-publish hentry category-pgtemplate-acf-blog-temporary page_template_type-cro2 page_template_type-national-rehab">
    <div className="elementor-element elementor-element-5e166ce6 e-flex e-con-boxed e-con e-parent e-lazyloaded" data-settings="{&quot;background_background&quot;:&quot;classic&quot;}">
      <div className="e-con-inner">
        <div className="elementor-element elementor-element-d752ce7 elementor-widget-mobile__width-auto elementor-absolute elementor-widget elementor-widget-image" data-settings="{&quot;_position&quot;:&quot;absolute&quot;}" data-widget_type="image.default">
          <div className="elementor-widget-container">
            <Image src="/images/735d7f4a434910a2b14b0530fca8d6c2.webp" alt="" width={365} height={537} className="attachment-full size-full wp-image-54516 entered error" />
          </div>
        </div>
        <div className="elementor-element elementor-element-77f6763 e-flex e-con-boxed e-con e-child">
          <div className="e-con-inner">
            <div className="elementor-element elementor-element-36246d20 e-con-full elementor-hidden-desktop elementor-hidden-tablet elementor-hidden-mobile e-flex e-con e-child">
              <div className="elementor-element elementor-element-46a0d57 elementor-widget elementor-widget-heading" data-widget_type="heading.default">
                <div className="elementor-widget-container">
                  <div className="elementor-heading-title elementor-size-default">
                    Inpatient Alcohol Rehab Treatment
                  </div>
                </div>
              </div>
              <div className="elementor-element elementor-element-686a2233 elementor-widget elementor-widget-heading" data-widget_type="heading.default">
                <div className="elementor-widget-container">
                  <div className="elementor-heading-title elementor-size-default">
                    Outpatient Alcohol Rehab Treatment
                  </div>
                </div>
              </div>
            </div>
            <div className="elementor-element elementor-element-7d9e7522 e-flex e-con-boxed e-con e-child">
              <div className="e-con-inner">
                <div className="elementor-element elementor-element-73c5fe60 elementor-widget elementor-widget-heading" data-widget_type="heading.default">
                  <div className="elementor-widget-container">
                    <h1 className="elementor-heading-title elementor-size-default" style={{"fontSize":"40px"}}>
                      {h1}
                    </h1>
                  </div>
                </div>
                <div className="elementor-element elementor-element-691f91f2 e-con-full e-flex e-con e-child">
                  <div className="elementor-element elementor-element-548cf3be elementor-widget elementor-widget-image" data-widget_type="image.default">
                    <div className="elementor-widget-container">
                      <Image src="/images/cbe1ff6e680ff2a501fe36fc8f2b7500.webp" alt="" width={95} height={90} className="attachment-thumbnail size-thumbnail wp-image-54407 entered error" />
                    </div>
                  </div>
                  <div className="elementor-element elementor-element-65c1c890 elementor-widget elementor-widget-image" data-widget_type="image.default">
                    <div className="elementor-widget-container">
                      <Image src="/images/3aca933a3c6e29944faab39776f794b5.webp" alt="" width={83} height={90} className="attachment-thumbnail size-thumbnail wp-image-54408 entered error" />
                    </div>
                  </div>
                  <div className="elementor-element elementor-element-1fa9838d elementor-widget elementor-widget-image" data-widget_type="image.default">
                    <div className="elementor-widget-container">
                      <Image src="/images/271622d5800857b84c5ffae18c6b99e1.webp" alt="" width={103} height={90} className="attachment-thumbnail size-thumbnail wp-image-54409 entered error" />
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="elementor-element elementor-element-0174648 e-con-full e-flex e-con e-child">
              <div className="elementor-element elementor-element-a568d67 elementor-author-box--align-left elementor-author-box--image-valign-middle elementor-widget-mobile__width-auto author-box-sty elementor-widget elementor-widget-author-box" data-widget_type="author-box.default">
                <div className="elementor-widget-container">
                  <div className="elementor-author-box">
                    <div className="elementor-author-box__avatar">
                      <Image src="/images/f85771f302c1dd037770156cc1d2d363.webp" alt="Picture of Written By:" width={64} height={64} className="entered error" />
                    </div>
                    <div className="elementor-author-box__text">
                      <div>
                        <h6 className="elementor-author-box__name" style={{"fontSize":"20px"}}>
                          Written By:
                        </h6>
                      </div>
                      <div className="elementor-author-box__bio">
                        <p style={{"fontSize":"16px"}}>
                          Matthew D'Ursov
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="elementor-element elementor-element-cbcaa6d elementor-author-box--align-left elementor-author-box--image-valign-middle elementor-widget-mobile__width-initial author-box-sty elementor-widget elementor-widget-author-box" data-widget_type="author-box.default">
                <div className="elementor-widget-container">
                  <div className="elementor-author-box">
                    <div className="elementor-author-box__avatar">
                      <Image src="/images/dc984c7e7ba3124d7a6306bb29d6cc72.webp" alt="Picture of Content Manager:" width={64} height={64} className="entered error" />
                    </div>
                    <div className="elementor-author-box__text">
                      <div>
                        <h6 className="elementor-author-box__name" style={{"fontSize":"20px"}}>
                          Content Manager:
                        </h6>
                      </div>
                      <div className="elementor-author-box__bio">
                        <p style={{"fontSize":"16px"}}>
                          Amy Leifeste
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="elementor-element elementor-element-3925faa elementor-author-box--align-left elementor-author-box--image-valign-middle elementor-widget__width-auto author-box-sty elementor-widget elementor-widget-author-box" data-widget_type="author-box.default">
                <div className="elementor-widget-container">
                  <div className="elementor-author-box">
                    <div className="elementor-author-box__avatar">
                      <Image src="/images/e1e03ab41ed66f6f59f84d4004a0902b.webp" alt="Picture of Editor:" width={64} height={64} className="entered error" />
                    </div>
                    <div className="elementor-author-box__text">
                      <div>
                        <h6 className="elementor-author-box__name" style={{"fontSize":"20px"}}>
                          Editor:
                        </h6>
                      </div>
                      <div className="elementor-author-box__bio">
                        <p style={{"fontSize":"16px"}}>
                          Karena Mathis
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="elementor-element elementor-element-e62861f elementor-absolute elementor-widget elementor-widget-image" data-settings="{&quot;_position&quot;:&quot;absolute&quot;}" data-widget_type="image.default">
          <div className="elementor-widget-container">
            <Image src="/images/7bf0b19e7905ce1d68945364479ddf7f.webp" alt="" width={364} height={537} className="attachment-full size-full wp-image-54517 entered error" />
          </div>
        </div>
      </div>
    </div>
    <div className="elementor-element elementor-element-bcb6f4f e-flex e-con-boxed e-con e-parent e-lazyloaded">
      <div className="e-con-inner">
        <div className="elementor-element elementor-element-452966b e-con-full e-flex e-con e-child" data-settings="{&quot;background_background&quot;:&quot;gradient&quot;}">
          <div className="elementor-element elementor-element-d9e6b71 e-con-full e-flex e-con e-child">
            <div className="elementor-element elementor-element-d933cd8 e-con-full e-flex e-con e-child">
              <div className="elementor-element elementor-element-5802b28 elementor-widget elementor-widget-heading" data-widget_type="heading.default">
                <div className="elementor-widget-container">
                  <h6 className="elementor-heading-title elementor-size-default" style={{"fontSize":"20px"}}>
                    We understand recovery, we can help
                  </h6>
                </div>
              </div>
              <div className="elementor-element elementor-element-3dec4f7 elementor-icon-list--layout-inline icon-list-sty elementor-list-item-link-full_width elementor-widget elementor-widget-icon-list" data-widget_type="icon-list.default">
                <div className="elementor-widget-container">
                  <ul className="elementor-icon-list-items elementor-inline-items">
                    <li className="elementor-icon-list-item elementor-inline-item" style={{"fontSize":"16px"}}>
                      <span className="elementor-icon-list-icon" style={{"fontSize":"16px"}}>
                        <svg width={24} height={24} viewBox="0 0 24 24" fill="none">
                          <mask id="mask0_607_817" style={{"maskType":"alpha"}} maskUnits="userSpaceOnUse" x="0" y="0" width={24} height={24}>
                            <rect width={24} height={24} fill="url(#pattern0_607_817)"></rect>
                          </mask>
                          <g mask="url(#mask0_607_817)">
                            <rect width={24} height={24} fill="#AED5BA"></rect>
                          </g>
                          <defs>
                            <pattern id="pattern0_607_817" patternContentUnits="objectBoundingBox" width={1} height={1}>
                              <use href="#image0_607_817" transform="scale(0.00195312)"></use>
                            </pattern>
                            <image id="image0_607_817" width={512} height={512} preserveAspectRatio="none" href="data:image/png;base64,"></image>
                          </defs>
                        </svg>
                      </span>
                      <span className="elementor-icon-list-text" style={{"fontSize":"16px"}}>
                        Personalized, Addiction Treatment{" "}
                        <br />
                        {" "}Programs for Every Individual
                      </span>
                    </li>
                    <li className="elementor-icon-list-item elementor-inline-item" style={{"fontSize":"16px"}}>
                      <span className="elementor-icon-list-icon" style={{"fontSize":"16px"}}>
                        <svg width={24} height={24} viewBox="0 0 24 24" fill="none">
                          <mask id="mask0_607_817" style={{"maskType":"alpha"}} maskUnits="userSpaceOnUse" x="0" y="0" width={24} height={24}>
                            <rect width={24} height={24} fill="url(#pattern0_607_817)"></rect>
                          </mask>
                          <g mask="url(#mask0_607_817)">
                            <rect width={24} height={24} fill="#AED5BA"></rect>
                          </g>
                          <defs>
                            <pattern id="pattern0_607_817" patternContentUnits="objectBoundingBox" width={1} height={1}>
                              <use href="#image0_607_817" transform="scale(0.00195312)"></use>
                            </pattern>
                            <image id="image0_607_817" width={512} height={512} preserveAspectRatio="none" href="data:image/png;base64,"></image>
                          </defs>
                        </svg>
                      </span>
                      <span className="elementor-icon-list-text" style={{"fontSize":"16px"}}>
                        Masters-level addiction therapists
                      </span>
                    </li>
                    <li className="elementor-icon-list-item elementor-inline-item" style={{"fontSize":"16px"}}>
                      <span className="elementor-icon-list-icon" style={{"fontSize":"16px"}}>
                        <svg width={24} height={24} viewBox="0 0 24 24" fill="none">
                          <mask id="mask0_607_817" style={{"maskType":"alpha"}} maskUnits="userSpaceOnUse" x="0" y="0" width={24} height={24}>
                            <rect width={24} height={24} fill="url(#pattern0_607_817)"></rect>
                          </mask>
                          <g mask="url(#mask0_607_817)">
                            <rect width={24} height={24} fill="#AED5BA"></rect>
                          </g>
                          <defs>
                            <pattern id="pattern0_607_817" patternContentUnits="objectBoundingBox" width={1} height={1}>
                              <use href="#image0_607_817" transform="scale(0.00195312)"></use>
                            </pattern>
                            <image id="image0_607_817" width={512} height={512} preserveAspectRatio="none" href="data:image/png;base64,"></image>
                          </defs>
                        </svg>
                      </span>
                      <span className="elementor-icon-list-text" style={{"fontSize":"16px"}}>
                        In network with most major insurances
                      </span>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
            <div className="elementor-element elementor-element-b646dba e-con-full e-flex e-con e-child">
              <div className="elementor-element elementor-element-4bf7678 elementor-align-justify elementor-widget-mobile__width-inherit elementor-widget elementor-widget-button" data-widget_type="button.default">
                <div className="elementor-widget-container">
                  <div className="elementor-button-wrapper">
                    <Link className="elementor-button elementor-button-link elementor-size-sm" href="tel:844-759-0999" style={{"fontSize":"16px"}}>
                      <span className="elementor-button-content-wrapper" style={{"fontSize":"16px"}}>
                        <span className="elementor-button-text" style={{"fontSize":"16px"}}>
                          Call Now! 844-759-0999
                        </span>
                      </span>
                    </Link>
                  </div>
                </div>
              </div>
              <div className="elementor-element elementor-element-4fc2b76 elementor-align-justify elementor-widget-mobile__width-inherit elementor-widget elementor-widget-button" data-widget_type="button.default">
                <div className="elementor-widget-container">
                  <div className="elementor-button-wrapper">
                    <Link className="elementor-button elementor-button-link elementor-size-sm" href="/check-your-insurance/" style={{"fontSize":"16px"}}>
                      <span className="elementor-button-content-wrapper" style={{"fontSize":"16px"}}>
                        <span className="elementor-button-text" style={{"fontSize":"16px"}}>
                          Verify Your Insurance
                        </span>
                      </span>
                    </Link>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
    {__hasData(["h2___con","h2___head"]) ? (
<div className="elementor-element elementor-element-c1e2776 table-of-content-headings e-flex e-con-boxed e-con e-parent e-lazyloaded">
      <div className="e-con-inner">
        <div className="elementor-element elementor-element-cc3a9e4 e-con-full e-flex e-con e-child">
          <div className="elementor-element elementor-element-ab97e6c table-of-content-sty elementor-widget elementor-widget-table-of-contents" data-settings="{&quot;headings_by_tags&quot;:[&quot;h2&quot;],&quot;exclude_headings_by_selector&quot;:[],&quot;no_headings_message&quot;:&quot;No headings were found on this page.&quot;,&quot;container&quot;:&quot;.table-of-content-headings&quot;,&quot;marker_view&quot;:&quot;numbers&quot;,&quot;hierarchical_view&quot;:&quot;yes&quot;,&quot;min_height&quot;:{&quot;unit&quot;:&quot;px&quot;,&quot;size&quot;:&quot;&quot;,&quot;sizes&quot;:[]},&quot;min_height_tablet&quot;:{&quot;unit&quot;:&quot;px&quot;,&quot;size&quot;:&quot;&quot;,&quot;sizes&quot;:[]},&quot;min_height_mobile&quot;:{&quot;unit&quot;:&quot;px&quot;,&quot;size&quot;:&quot;&quot;,&quot;sizes&quot;:[]}}">
            <div className="elementor-widget-container">
              <div className="elementor-toc__header">
                <h4 className="elementor-toc__header-title" style={{"fontSize":"20px"}}>
                  Table of Contents
                </h4>
              </div>
              <BlogTocList column=".elementor-element-cc3a9e4" />
            </div>
          </div>
        </div>
        <div className="elementor-element elementor-element-74d3724 e-con-full e-flex e-con e-child">
          <div className="elementor-element elementor-element-ef0957c elementor-widget elementor-widget-heading" data-widget_type="heading.default">
            <div className="elementor-widget-container">
              <h2 className="elementor-heading-title elementor-size-default" style={{"fontSize":"32px"}}>
                {h2___head}
              </h2>
            </div>
          </div>
          <div className="elementor-element elementor-element-e5152d5 elementor-widget elementor-widget-text-editor" data-widget_type="text-editor.default">
            <div className="elementor-widget-container">
              <RichText tocSource style={{"fontSize":"16px"}} html={h2___con} />
            </div>
          </div>
        </div>
      </div>
    </div>
) : null}
    {__present("cd87278") ? (
<div className="elementor-element elementor-element-cd87278 e-con-full e-flex e-con e-parent e-lazyloaded">
      <div className="elementor-element elementor-element-8b7faf6 elementor-widget elementor-widget-template" data-widget_type="template.default">
        <div className="elementor-widget-container">
          <div className="elementor-template">
            <div className="elementor elementor-54723">
              <div className="elementor-element elementor-element-7b995ec1 e-flex e-con-boxed e-con e-parent e-lazyloaded" data-settings="{&quot;background_background&quot;:&quot;classic&quot;}">
                <div className="e-con-inner">
                  <div className="elementor-element elementor-element-38726823 e-con-full e-flex e-con e-child">
                    <div className="elementor-element elementor-element-50aea734 elementor-widget elementor-widget-heading" data-widget_type="heading.default">
                      <div className="elementor-widget-container">
                        <h3 className="elementor-heading-title elementor-size-default" style={{"fontSize":"32px"}}>
                          Get Personalized Addiction Care at The {brand}
                        </h3>
                      </div>
                    </div>
                    <div className="elementor-element elementor-element-11be573e elementor-widget elementor-widget-text-editor" data-widget_type="text-editor.default">
                      <div className="elementor-widget-container">
                        <p style={{"fontSize":"16px"}}>
                          Our rehab centers help men and women who are struggling with addictions, substance abuse and mental health disorders. We also provide treatment exclusively to International Association of Fire Fighters (IAFF) members at our Maryland facility, the IAFF Center of Excellence for Behavioral Health Treatment and Recovery.
                        </p>
                      </div>
                    </div>
                  </div>
                  <div className="elementor-element elementor-element-63bfb9a e-con-full e-flex e-con e-child" data-settings="{&quot;background_background&quot;:&quot;classic&quot;}"></div>
                  <div className="elementor-element elementor-element-0fd66de e-con-full e-flex e-con e-child">
                    <div className="elementor-element elementor-element-af82704 elementor-align-justify elementor-widget-mobile__width-inherit elementor-widget elementor-widget-button" data-widget_type="button.default">
                      <div className="elementor-widget-container">
                        <div className="elementor-button-wrapper">
                          <Link className="elementor-button elementor-button-link elementor-size-sm" href="/our-facilities/" style={{"fontSize":"16px"}}>
                            <span className="elementor-button-content-wrapper" style={{"fontSize":"16px"}}>
                              <span className="elementor-button-text" style={{"fontSize":"16px"}}>
                                Find a Location
                              </span>
                            </span>
                          </Link>
                        </div>
                      </div>
                    </div>
                    <div className="elementor-element elementor-element-1ca76f7 elementor-align-justify elementor-widget-mobile__width-inherit elementor-widget elementor-widget-button" data-widget_type="button.default">
                      <div className="elementor-widget-container">
                        <div className="elementor-button-wrapper">
                          <Link className="elementor-button elementor-button-link elementor-size-sm" href="/rehab-admissions/" style={{"fontSize":"16px"}}>
                            <span className="elementor-button-content-wrapper" style={{"fontSize":"16px"}}>
                              <span className="elementor-button-text" style={{"fontSize":"16px"}}>
                                See Our Admissions Process
                              </span>
                            </span>
                          </Link>
                        </div>
                      </div>
                    </div>
                    <div className="elementor-element elementor-element-eec62b6 elementor-align-justify elementor-widget-mobile__width-inherit elementor-widget elementor-widget-button" data-widget_type="button.default">
                      <div className="elementor-widget-container">
                        <div className="elementor-button-wrapper">
                          <Link className="elementor-button elementor-button-link elementor-size-sm" href="/reviews/" style={{"fontSize":"16px"}}>
                            <span className="elementor-button-content-wrapper" style={{"fontSize":"16px"}}>
                              <span className="elementor-button-text" style={{"fontSize":"16px"}}>
                                Read Our Success Stories
                              </span>
                            </span>
                          </Link>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
) : null}
    {__hasData(["blog_section_1___con"]) ? (
<div className="elementor-element elementor-element-2f5adab table-of-content-headings empty-container-sty e-flex e-con-boxed e-con e-parent e-lazyloaded">
      <div className="e-con-inner">
        <div className="elementor-element elementor-element-e44e46f cro-bog-content-sty elementor-widget elementor-widget-text-editor" data-widget_type="text-editor.default">
          <div className="elementor-widget-container">
              <div data-toc-source dangerouslySetInnerHTML={{ __html: blog_section_1___con }} />
            </div>
        </div>
      </div>
    </div>
) : null}
    <div className="elementor-element elementor-element-447eff3 e-con-full elementor-hidden-desktop elementor-hidden-tablet elementor-hidden-mobile e-flex e-con e-parent">
      <div className="elementor-element elementor-element-d3f2551 elementor-widget elementor-widget-template" data-widget_type="template.default">
        <div className="elementor-widget-container">
          <div className="elementor-template">
            <div className="elementor elementor-54594">
              <div className="elementor-element elementor-element-448a5ab2 e-flex e-con-boxed e-con e-parent" data-settings="{&quot;background_background&quot;:&quot;classic&quot;}">
                <div className="e-con-inner">
                  <div className="elementor-element elementor-element-39178f4 e-con-full e-flex e-con e-child">
                    <div className="elementor-element elementor-element-680294cd elementor-widget__width-initial elementor-widget-mobile__width-auto elementor-widget elementor-widget-heading" data-widget_type="heading.default">
                      <div className="elementor-widget-container">
                        <h3 className="elementor-heading-title elementor-size-default" style={{"fontSize":"32px"}}>
                          Related Blogs
                        </h3>
                      </div>
                    </div>
                    <div className="elementor-element elementor-element-1e42ac7f elementor-widget elementor-widget-text-editor" data-widget_type="text-editor.default">
                      <div className="elementor-widget-container">
                        <p style={{"fontSize":"16px"}}>
                          Check out our addiction recovery blog to learn more about substance use disorders and how to get effective treatment.
                        </p>
                      </div>
                    </div>
                  </div>
                  <div className="elementor-element elementor-element-375ff5df e-con-full e-flex e-con e-child">
                    <div className="elementor-element elementor-element-7dc87a1d elementor-widget elementor-widget-loop-carousel e-widget-swiper" data-settings="{&quot;template_id&quot;:&quot;54492&quot;,&quot;image_spacing_custom&quot;:{&quot;unit&quot;:&quot;px&quot;,&quot;size&quot;:48,&quot;sizes&quot;:[]},&quot;image_spacing_custom_tablet&quot;:{&quot;unit&quot;:&quot;px&quot;,&quot;size&quot;:24,&quot;sizes&quot;:[]},&quot;_skin&quot;:&quot;post&quot;,&quot;slides_to_show&quot;:&quot;3&quot;,&quot;slides_to_show_tablet&quot;:&quot;2&quot;,&quot;slides_to_show_mobile&quot;:&quot;1&quot;,&quot;slides_to_scroll&quot;:&quot;1&quot;,&quot;edit_handle_selector&quot;:&quot;.elementor-loop-container&quot;,&quot;autoplay&quot;:&quot;yes&quot;,&quot;autoplay_speed&quot;:5000,&quot;pause_on_hover&quot;:&quot;yes&quot;,&quot;pause_on_interaction&quot;:&quot;yes&quot;,&quot;infinite&quot;:&quot;yes&quot;,&quot;speed&quot;:500,&quot;offset_sides&quot;:&quot;none&quot;,&quot;image_spacing_custom_mobile&quot;:{&quot;unit&quot;:&quot;px&quot;,&quot;size&quot;:&quot;&quot;,&quot;sizes&quot;:[]}}" data-widget_type="loop-carousel.post">
                      <div className="elementor-widget-container">
                        <div className="swiper elementor-loop-container elementor-grid swiper-initialized swiper-horizontal swiper-pointer-events" role="list" dir="ltr">
                          <div className="swiper-wrapper" aria-live="off" id="swiper-wrapper-135105228768f5acd" style={{"transitionDuration":"0ms","transform":"none"}}>
                            <div className="elementor elementor-54492 swiper-slide e-loop-item e-loop-item-54868 post-54868 page type-page status-publish has-post-thumbnail hentry category-pgtemplate-acf-blog page_template_type-blog page_template_type-cro1-blog" role="group" aria-roledescription="slide" aria-label="1 of 6" style={{"visibility":"visible"}}>
                              <div className="elementor-element elementor-element-12be248 e-flex e-con-boxed e-con e-parent">
                                <div className="e-con-inner">
                                  <div className="elementor-element elementor-element-d279927 elementor-widget elementor-widget-image" data-widget_type="image.default">
                                    <div className="elementor-widget-container">
                                      <Image src="/images/00ce44d668da2f26957081222f71256c.webp" alt="" width={1920} height={436} className="attachment-full size-full wp-image-54438" />
                                    </div>
                                  </div>
                                  <div className="elementor-element elementor-element-c2193c0 elementor-widget elementor-widget-heading" data-widget_type="heading.default">
                                    <div className="elementor-widget-container">
                                      <h3 className="elementor-heading-title elementor-size-default" style={{"fontSize":"26px"}}>
                                        <Link href="/mental-health/therapy/does-insurance-cover-residential-mental-health-treatment/" style={{"fontSize":"26px"}}>
                                          Does Insurance Cover Residential Mental Health Treatment?
                                        </Link>
                                      </h3>
                                    </div>
                                  </div>
                                  <div className="elementor-element elementor-element-d0395f8 elementor-widget elementor-widget-text-editor" data-widget_type="text-editor.default">
                                    <div className="elementor-widget-container">
                                      <p style={{"fontSize":"16px"}}>
                                        Insurance may cover residential mental health treatment, but coverage depends heavily on your specific insurance plan, clinical needs, and how care is classified. In many
                                      </p>
                                    </div>
                                  </div>
                                </div>
                              </div>
                            </div>
                            <div className="elementor elementor-54492 swiper-slide e-loop-item e-loop-item-6666 post-6666 page type-page status-publish hentry category-pgtemplate-acf-blog page_template_type-blog page_template_type-cro1-blog" role="group" aria-roledescription="slide" aria-label="2 of 6" style={{"visibility":"visible"}}>
                              <div className="elementor-element elementor-element-12be248 e-flex e-con-boxed e-con e-parent">
                                <div className="e-con-inner">
                                  <div className="elementor-element elementor-element-d279927 elementor-widget elementor-widget-image" data-widget_type="image.default">
                                    <div className="elementor-widget-container">
                                      <Image src="/images/cdb59969d24ca38247e7d73f6a0d5d3f.webp" alt="" width={1920} height={1280} className="attachment-full size-full wp-image-54442" />
                                    </div>
                                  </div>
                                  <div className="elementor-element elementor-element-c2193c0 elementor-widget elementor-widget-heading" data-widget_type="heading.default">
                                    <div className="elementor-widget-container">
                                      <h3 className="elementor-heading-title elementor-size-default" style={{"fontSize":"26px"}}>
                                        <Link href="/mental-health/personality-disorders/paranoid-personality-disorder/why-do-i-feel-like-everyone-hates-me/" style={{"fontSize":"26px"}}>
                                          Why Do I Feel Like Everyone Hates Me?
                                        </Link>
                                      </h3>
                                    </div>
                                  </div>
                                  <div className="elementor-element elementor-element-d0395f8 elementor-widget elementor-widget-text-editor" data-widget_type="text-editor.default">
                                    <div className="elementor-widget-container">
                                      Hydromorphone, marketed under the brand names Dilaudid and Exalgo, is a potent semi-synthetic opioid pain reliever utilized in hospital settings and as a component of long-term treatment plans for individuals who are tolerant to opioids and experiencing severe pain.
                                    </div>
                                  </div>
                                </div>
                              </div>
                            </div>
                            <div className="elementor elementor-54492 swiper-slide e-loop-item e-loop-item-9669 post-9669 page type-page status-publish hentry category-pgtemplate-acf-blog page_template_type-blog page_template_type-cro1-blog" role="group" aria-roledescription="slide" aria-label="3 of 6" style={{"visibility":"visible"}}>
                              <div className="elementor-element elementor-element-12be248 e-flex e-con-boxed e-con e-parent">
                                <div className="e-con-inner">
                                  <div className="elementor-element elementor-element-d279927 elementor-widget elementor-widget-image" data-widget_type="image.default">
                                    <div className="elementor-widget-container">
                                      <Image src="/images/cdb59969d24ca38247e7d73f6a0d5d3f.webp" alt="" width={1920} height={1280} className="attachment-full size-full wp-image-54442" />
                                    </div>
                                  </div>
                                  <div className="elementor-element elementor-element-c2193c0 elementor-widget elementor-widget-heading" data-widget_type="heading.default">
                                    <div className="elementor-widget-container">
                                      <h3 className="elementor-heading-title elementor-size-default" style={{"fontSize":"26px"}}>
                                        <Link href="/mental-health/overstimulated-meaning/" style={{"fontSize":"26px"}}>
                                          Overstimulated Meaning
                                        </Link>
                                      </h3>
                                    </div>
                                  </div>
                                  <div className="elementor-element elementor-element-d0395f8 elementor-widget elementor-widget-text-editor" data-widget_type="text-editor.default">
                                    <div className="elementor-widget-container">
                                      Hydromorphone, marketed under the brand names Dilaudid and Exalgo, is a potent semi-synthetic opioid pain reliever utilized in hospital settings and as a component of long-term treatment plans for individuals who are tolerant to opioids and experiencing severe pain.
                                    </div>
                                  </div>
                                </div>
                              </div>
                            </div>
                            <div className="elementor elementor-54492 swiper-slide e-loop-item e-loop-item-9667 post-9667 page type-page status-publish hentry category-pgtemplate-acf-blog page_template_type-blog page_template_type-cro1-blog" role="group" aria-roledescription="slide" aria-label="4 of 6" style={{"visibility":"visible"}}>
                              <div className="elementor-element elementor-element-12be248 e-flex e-con-boxed e-con e-parent">
                                <div className="e-con-inner">
                                  <div className="elementor-element elementor-element-d279927 elementor-widget elementor-widget-image" data-widget_type="image.default">
                                    <div className="elementor-widget-container">
                                      <Image src="/images/cdb59969d24ca38247e7d73f6a0d5d3f.webp" alt="" width={1920} height={1280} className="attachment-full size-full wp-image-54442" />
                                    </div>
                                  </div>
                                  <div className="elementor-element elementor-element-c2193c0 elementor-widget elementor-widget-heading" data-widget_type="heading.default">
                                    <div className="elementor-widget-container">
                                      <h3 className="elementor-heading-title elementor-size-default" style={{"fontSize":"26px"}}>
                                        <Link href="/mental-health/what-to-expect-during-inpatient-mental-health-stay/" style={{"fontSize":"26px"}}>
                                          What to Expect During Inpatient Mental Health Stay
                                        </Link>
                                      </h3>
                                    </div>
                                  </div>
                                  <div className="elementor-element elementor-element-d0395f8 elementor-widget elementor-widget-text-editor" data-widget_type="text-editor.default">
                                    <div className="elementor-widget-container">
                                      Hydromorphone, marketed under the brand names Dilaudid and Exalgo, is a potent semi-synthetic opioid pain reliever utilized in hospital settings and as a component of long-term treatment plans for individuals who are tolerant to opioids and experiencing severe pain.
                                    </div>
                                  </div>
                                </div>
                              </div>
                            </div>
                            <div className="elementor elementor-54492 swiper-slide e-loop-item e-loop-item-9665 post-9665 page type-page status-publish hentry category-pgtemplate-acf-blog page_template_type-blog page_template_type-cro1-blog" role="group" aria-roledescription="slide" aria-label="5 of 6" style={{"visibility":"visible"}}>
                              <div className="elementor-element elementor-element-12be248 e-flex e-con-boxed e-con e-parent">
                                <div className="e-con-inner">
                                  <div className="elementor-element elementor-element-d279927 elementor-widget elementor-widget-image" data-widget_type="image.default">
                                    <div className="elementor-widget-container">
                                      <Image src="/images/cdb59969d24ca38247e7d73f6a0d5d3f.webp" alt="" width={1920} height={1280} className="attachment-full size-full wp-image-54442" />
                                    </div>
                                  </div>
                                  <div className="elementor-element elementor-element-c2193c0 elementor-widget elementor-widget-heading" data-widget_type="heading.default">
                                    <div className="elementor-widget-container">
                                      <h3 className="elementor-heading-title elementor-size-default" style={{"fontSize":"26px"}}>
                                        <Link href="/mental-health/residential-vs-inpatient-mental-health-treatment/" style={{"fontSize":"26px"}}>
                                          Residential vs Inpatient Mental Health Treatment
                                        </Link>
                                      </h3>
                                    </div>
                                  </div>
                                  <div className="elementor-element elementor-element-d0395f8 elementor-widget elementor-widget-text-editor" data-widget_type="text-editor.default">
                                    <div className="elementor-widget-container">
                                      Hydromorphone, marketed under the brand names Dilaudid and Exalgo, is a potent semi-synthetic opioid pain reliever utilized in hospital settings and as a component of long-term treatment plans for individuals who are tolerant to opioids and experiencing severe pain.
                                    </div>
                                  </div>
                                </div>
                              </div>
                            </div>
                            <div className="elementor elementor-54492 swiper-slide e-loop-item e-loop-item-9663 post-9663 page type-page status-publish hentry category-pgtemplate-acf-blog page_template_type-blog page_template_type-cro1-blog" role="group" aria-roledescription="slide" aria-label="6 of 6" style={{"visibility":"visible"}}>
                              <div className="elementor-element elementor-element-12be248 e-flex e-con-boxed e-con e-parent">
                                <div className="e-con-inner">
                                  <div className="elementor-element elementor-element-d279927 elementor-widget elementor-widget-image" data-widget_type="image.default">
                                    <div className="elementor-widget-container">
                                      <Image src="/images/cdb59969d24ca38247e7d73f6a0d5d3f.webp" alt="" width={1920} height={1280} className="attachment-full size-full wp-image-54442" />
                                    </div>
                                  </div>
                                  <div className="elementor-element elementor-element-c2193c0 elementor-widget elementor-widget-heading" data-widget_type="heading.default">
                                    <div className="elementor-widget-container">
                                      <h3 className="elementor-heading-title elementor-size-default" style={{"fontSize":"26px"}}>
                                        <Link href="/mental-health/can-i-check-myself-into-a-mental-hospital/" style={{"fontSize":"26px"}}>
                                          can I check myself into a mental hospital
                                        </Link>
                                      </h3>
                                    </div>
                                  </div>
                                  <div className="elementor-element elementor-element-d0395f8 elementor-widget elementor-widget-text-editor" data-widget_type="text-editor.default">
                                    <div className="elementor-widget-container">
                                      Hydromorphone, marketed under the brand names Dilaudid and Exalgo, is a potent semi-synthetic opioid pain reliever utilized in hospital settings and as a component of long-term treatment plans for individuals who are tolerant to opioids and experiencing severe pain.
                                    </div>
                                  </div>
                                </div>
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="elementor-element elementor-element-e632a52 elementor-align-center elementor-widget-mobile__width-inherit elementor-mobile-align-center elementor-widget elementor-widget-button" data-widget_type="button.default">
                    <div className="elementor-widget-container">
                      <div className="elementor-button-wrapper">
                        <Link className="elementor-button elementor-button-link elementor-size-sm" href="/mental-health/blog/" style={{"fontSize":"16px"}}>
                          <span className="elementor-button-content-wrapper" style={{"fontSize":"16px"}}>
                            <span className="elementor-button-text" style={{"fontSize":"16px"}}>
                              View Other Topics
                            </span>
                          </span>
                        </Link>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
    <div className="elementor-element elementor-element-d7e4e9c e-con-full e-flex e-con e-parent e-lazyloaded" id="#reviews">
      <div className="elementor-element elementor-element-5c7f0ad elementor-widget elementor-widget-template" data-widget_type="template.default">
        <div className="elementor-widget-container">
          <div className="elementor-template">
            <div className="elementor elementor-54598">
              <div className="elementor-element elementor-element-7bf01276 e-flex e-con-boxed e-con e-parent e-lazyloaded" id="#reviews" data-settings="{&quot;background_background&quot;:&quot;gradient&quot;}">
                <div className="e-con-inner">
                  <div className="elementor-element elementor-element-4408da16 elementor-widget elementor-widget-html" data-widget_type="html.default">
                    <div className="elementor-widget-container">
                      <TrustIndexWidget widgetId="6ba854c40139851d40860ccf7a9" />
                    </div>
                  </div>
                  <div className="elementor-element elementor-element-4b2ad360 elementor-widget__width-initial elementor-widget elementor-widget-heading" data-widget_type="heading.default">
                    <div className="elementor-widget-container">
                      <h3 className="elementor-heading-title elementor-size-default" style={{"fontSize":"40px"}}>
                        What Our Clients Say
                      </h3>
                    </div>
                  </div>
                  <div className="elementor-element elementor-element-4db4a6db elementor-widget__width-initial elementor-widget elementor-widget-heading" data-widget_type="heading.default">
                    <div className="elementor-widget-container">
                      <h2 className="elementor-heading-title elementor-size-default" style={{"fontSize":"20px"}}>
                        5 Star Google Reviews
                      </h2>
                    </div>
                  </div>
                  <div className="elementor-element elementor-element-df8b2bf elementor-widget elementor-widget-menu-anchor" data-widget_type="menu-anchor.default">
                    <div className="elementor-widget-container">
                      <div className="elementor-menu-anchor" id="reviews"></div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
    <div className="elementor-element elementor-element-7228e26 e-con-full e-flex e-con e-parent e-lazyloaded">
      <div className="elementor-element elementor-element-87192bd elementor-widget elementor-widget-template" data-widget_type="template.default">
        <div className="elementor-widget-container">
          <div className="elementor-template">
            <div className="elementor elementor-54602">
              <div className="elementor-element elementor-element-76dfd324 e-flex e-con-boxed e-con e-parent e-lazyloaded">
                <div className="e-con-inner">
                  <div className="elementor-element elementor-element-55f0ab49 e-con-full e-flex e-con e-child">
                    <div className="elementor-element elementor-element-43f2325d e-con-full e-flex e-con e-child">
                      <div className="elementor-element elementor-element-3f9e6eb8 elementor-widget__width-initial elementor-widget-mobile__width-auto elementor-widget elementor-widget-heading" data-widget_type="heading.default">
                        <div className="elementor-widget-container">
                          <h3 className="elementor-heading-title elementor-size-default" style={{"fontSize":"32px"}}>
                            Where You’ll Be Staying
                          </h3>
                        </div>
                      </div>
                      <div className="elementor-element elementor-element-d81f27b elementor-widget__width-initial elementor-widget elementor-widget-text-editor" data-widget_type="text-editor.default">
                        <div className="elementor-widget-container">
                          <p style={{"fontSize":"16px"}}>
                            We strive to provide a comfortable, home-like environment during your recovery journey. Browse our facility images below to take a look at our upscale residential facilities.
                          </p>
                        </div>
                      </div>
                    </div>
                    <div className="elementor-element elementor-element-e9c3b2d e-con-full elementor-hidden-mobile e-flex e-con e-child">
                      <div className="elementor-element elementor-element-ee4eb26 elementor-align-justify elementor-widget-mobile__width-inherit elementor-hidden-mobile elementor-widget elementor-widget-button" data-widget_type="button.default">
                        <div className="elementor-widget-container">
                          <div className="elementor-button-wrapper">
                            <Link className="elementor-button elementor-button-link elementor-size-sm" href="/our-facilities/" style={{"fontSize":"16px"}}>
                              <span className="elementor-button-content-wrapper" style={{"fontSize":"16px"}}>
                                <span className="elementor-button-text" style={{"fontSize":"16px"}}>
                                  Tour Our Facilities
                                </span>
                              </span>
                            </Link>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="elementor-element elementor-element-db5b86e elementor-arrows-position-outside img-carousel-sty elementor-pagination-position-outside elementor-widget elementor-widget-image-carousel e-widget-swiper" data-settings="{&quot;slides_to_show&quot;:&quot;3&quot;,&quot;image_spacing_custom&quot;:{&quot;unit&quot;:&quot;px&quot;,&quot;size&quot;:48,&quot;sizes&quot;:[]},&quot;image_spacing_custom_tablet&quot;:{&quot;unit&quot;:&quot;px&quot;,&quot;size&quot;:24,&quot;sizes&quot;:[]},&quot;navigation&quot;:&quot;both&quot;,&quot;autoplay&quot;:&quot;yes&quot;,&quot;pause_on_hover&quot;:&quot;yes&quot;,&quot;pause_on_interaction&quot;:&quot;yes&quot;,&quot;autoplay_speed&quot;:5000,&quot;infinite&quot;:&quot;yes&quot;,&quot;speed&quot;:500,&quot;image_spacing_custom_mobile&quot;:{&quot;unit&quot;:&quot;px&quot;,&quot;size&quot;:&quot;&quot;,&quot;sizes&quot;:[]}}" data-widget_type="image-carousel.default">
                    <div className="elementor-widget-container">
                      <div className="elementor-image-carousel-wrapper swiper swiper-initialized swiper-horizontal swiper-pointer-events swiper-backface-hidden" role="region" aria-roledescription="carousel" aria-label="Image Carousel" dir="ltr">
                        <div className="elementor-image-carousel swiper-wrapper swiper-image-stretch" aria-live="off" id="swiper-wrapper-25687e1ea814a104" style={{"transform":"none","transitionDuration":"0ms"}}>
                          <div className="swiper-slide swiper-slide-prev" role="group" aria-roledescription="slide" aria-label="1 / 4" style={{"width":"369.333px","marginRight":"48px","visibility":"visible"}}>
                            <Link data-elementor-open-lightbox="yes" href="/wp-content/uploads/2026/01/rectangle-250-1-68b941b299489.webp" style={{"fontSize":"16px"}}>
                              <figure className="swiper-slide-inner">
                                <Image src="/images/23e32effef01be710b967e625e1cf775.webp" alt="rectangle-250-1-68b941b299489.webp" width={1200} height={669} className="swiper-slide-image entered exited" />
                              </figure>
                            </Link>
                          </div>
                          <div className="swiper-slide swiper-slide-active" role="group" aria-roledescription="slide" aria-label="2 / 4" style={{"width":"369.333px","marginRight":"48px","visibility":"visible"}}>
                            <Link data-elementor-open-lightbox="yes" href="/wp-content/uploads/2026/01/rectangle-251-1-1-68b941b47d396.webp" style={{"fontSize":"16px"}}>
                              <figure className="swiper-slide-inner">
                                <Image src="/images/53730dd809d76bf3c4ee8d4c6192136f.webp" alt="rectangle-251-1-1-68b941b47d396.webp" width={1200} height={669} className="swiper-slide-image entered exited" />
                              </figure>
                            </Link>
                          </div>
                          <div className="swiper-slide swiper-slide-next" role="group" aria-roledescription="slide" aria-label="3 / 4" style={{"width":"369.333px","marginRight":"48px","visibility":"visible"}}>
                            <Link data-elementor-open-lightbox="yes" href="/wp-content/uploads/2026/01/rectangle-250-2-68b941b9428a8.webp" style={{"fontSize":"16px"}}>
                              <figure className="swiper-slide-inner">
                                <Image src="/images/8c6bc9fc5ffdc0e4b55f4fd4cd84435e.webp" alt="rectangle-250-2-68b941b9428a8.webp" width={1200} height={669} className="swiper-slide-image entered exited" />
                              </figure>
                            </Link>
                          </div>
                          <div className="swiper-slide" role="group" aria-roledescription="slide" aria-label="4 / 4" style={{"width":"369.333px","marginRight":"48px","visibility":"visible"}}>
                            <Link data-elementor-open-lightbox="yes" href="/wp-content/uploads/2026/01/rectangle-251-2-68b941bad8a4b.webp" style={{"fontSize":"16px"}}>
                              <figure className="swiper-slide-inner">
                                <Image src="/images/d170e5150e665f38cabc754c85b76d78.webp" alt="rectangle-251-2-68b941bad8a4b.webp" width={1200} height={669} className="swiper-slide-image" />
                              </figure>
                            </Link>
                          </div>
                        </div>
                        <div className="elementor-swiper-button elementor-swiper-button-prev" role="button" tabIndex={0} aria-label="Previous slide" aria-controls="swiper-wrapper-25687e1ea814a104">
                          <svg aria-hidden="true" className="e-font-icon-svg e-eicon-chevron-left" viewBox="0 0 1000 1000">
                            <path d="M646 125C629 125 613 133 604 142L308 442C296 454 292 471 292 487 292 504 296 521 308 533L604 854C617 867 629 875 646 875 663 875 679 871 692 858 704 846 713 829 713 812 713 796 708 779 692 767L438 487 692 225C700 217 708 204 708 187 708 171 704 154 692 142 675 129 663 125 646 125Z"></path>
                          </svg>
                        </div>
                        <div className="elementor-swiper-button elementor-swiper-button-next" role="button" tabIndex={0} aria-label="Next slide" aria-controls="swiper-wrapper-25687e1ea814a104">
                          <svg aria-hidden="true" className="e-font-icon-svg e-eicon-chevron-right" viewBox="0 0 1000 1000">
                            <path d="M696 533C708 521 713 504 713 487 713 471 708 454 696 446L400 146C388 133 375 125 354 125 338 125 325 129 313 142 300 154 292 171 292 187 292 204 296 221 308 233L563 492 304 771C292 783 288 800 288 817 288 833 296 850 308 863 321 871 338 875 354 875 371 875 388 867 400 854L696 533Z"></path>
                          </svg>
                        </div>
                        <div className="swiper-pagination swiper-pagination-clickable swiper-pagination-bullets swiper-pagination-horizontal"></div>
                      </div>
                    </div>
                  </div>
                  <div className="elementor-element elementor-element-40ebb73d elementor-align-justify elementor-widget-mobile__width-inherit elementor-hidden-desktop elementor-hidden-tablet elementor-widget elementor-widget-button" data-widget_type="button.default">
                    <div className="elementor-widget-container">
                      <div className="elementor-button-wrapper">
                        <Link className="elementor-button elementor-button-link elementor-size-sm" href="/our-facilities/" style={{"fontSize":"16px"}}>
                          <span className="elementor-button-content-wrapper" style={{"fontSize":"16px"}}>
                            <span className="elementor-button-text" style={{"fontSize":"16px"}}>
                              Tour Our Facilities
                            </span>
                          </span>
                        </Link>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
    <div className="elementor-element elementor-element-dcac4ce e-con-full e-flex e-con e-parent e-lazyloaded">
      <div className="elementor-element elementor-element-045b41e elementor-widget elementor-widget-template" data-widget_type="template.default">
        <div className="elementor-widget-container">
          <div className="elementor-template">
            <div className="elementor elementor-54606">
              <div className="elementor-element elementor-element-68843b7c e-flex e-con-boxed e-con e-parent e-lazyloaded" data-settings="{&quot;background_background&quot;:&quot;gradient&quot;}">
                <div className="e-con-inner">
                  <div className="elementor-element elementor-element-7a9358b8 e-con-full e-flex e-con e-child">
                    <div className="elementor-element elementor-element-66a59cc e-con-full e-flex e-con e-child">
                      <div className="elementor-element elementor-element-35a02a00 elementor-widget elementor-widget-text-editor" data-widget_type="text-editor.default">
                        <div className="elementor-widget-container">
                          <p style={{"fontSize":"16px"}}>
                            Paying Out Of Pocket without Insurance
                          </p>
                        </div>
                      </div>
                      <div className="elementor-element elementor-element-1d8a3822 elementor-widget elementor-widget-heading" data-widget_type="heading.default">
                        <div className="elementor-widget-container">
                          <h4 className="elementor-heading-title elementor-size-default" style={{"fontSize":"32px"}}>
                            Range $5,000 to $60,000
                          </h4>
                        </div>
                      </div>
                    </div>
                    <div className="elementor-element elementor-element-1b81528 elementor-widget elementor-widget-image" data-widget_type="image.default">
                      <div className="elementor-widget-container">
                        <Image src="/images/af5d31ee07bff209e89a0b3487eb5b05.webp" alt="" width={366} height={257} className="attachment-full size-full wp-image-54927 entered exited" />
                      </div>
                    </div>
                  </div>
                  <div className="elementor-element elementor-element-22a59142 e-con-full e-flex e-con e-child" data-settings="{&quot;background_background&quot;:&quot;classic&quot;}">
                    <div className="elementor-element elementor-element-6938cb09 elementor-widget elementor-widget-text-editor" data-widget_type="text-editor.default">
                      <div className="elementor-widget-container">
                        <p style={{"fontSize":"16px"}}>
                          If you can’t pay your co-pay amount
                        </p>
                      </div>
                    </div>
                    <div className="elementor-element elementor-element-5fd41b5 elementor-align-justify elementor-widget-mobile__width-inherit elementor-widget elementor-widget-button" data-widget_type="button.default">
                      <div className="elementor-widget-container">
                        <div className="elementor-button-wrapper">
                          <Link className="elementor-button elementor-button-link elementor-size-sm" href="tel:844-759-0999" style={{"fontSize":"16px"}}>
                            <span className="elementor-button-content-wrapper" style={{"fontSize":"16px"}}>
                              <span className="elementor-button-text" style={{"fontSize":"16px"}}>
                                Call us we will work with you
                              </span>
                            </span>
                          </Link>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="elementor-element elementor-element-6a8663ef e-con-full e-flex e-con e-child">
                    <div className="elementor-element elementor-element-4fa52fa e-con-full e-flex e-con e-child">
                      <div className="elementor-element elementor-element-4afb1032 elementor-widget elementor-widget-text-editor" data-widget_type="text-editor.default">
                        <div className="elementor-widget-container">
                          <p style={{"fontSize":"16px"}}>
                            Paying Out Of Pocket with Insurance
                          </p>
                        </div>
                      </div>
                      <div className="elementor-element elementor-element-44cd3179 elementor-widget elementor-widget-heading" data-widget_type="heading.default">
                        <div className="elementor-widget-container">
                          <h4 className="elementor-heading-title elementor-size-default" style={{"fontSize":"32px"}}>
                            Range $0 to $12,000
                          </h4>
                        </div>
                      </div>
                      <div className="elementor-element elementor-element-8b84dcf elementor-widget elementor-widget-text-editor" data-widget_type="text-editor.default">
                        <div className="elementor-widget-container">
                          <p style={{"fontSize":"16px"}}>
                            (Depending on insurance plan)
                          </p>
                        </div>
                      </div>
                    </div>
                    <div className="elementor-element elementor-element-3eb9612 elementor-widget elementor-widget-image" data-widget_type="image.default">
                      <div className="elementor-widget-container">
                        <Image src="/images/b3a4e9d02d2520b82a9b37253329719c.webp" alt="" width={366} height={257} className="attachment-full size-full wp-image-54929 entered exited" />
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
    <div className="elementor-element elementor-element-0d20403 e-con-full e-flex e-con e-parent e-lazyloaded">
      <div className="elementor-element elementor-element-c613c3b elementor-widget elementor-widget-template" data-widget_type="template.default">
        <div className="elementor-widget-container">
          <div className="elementor-template">
            <div className="elementor elementor-54610">
              <div className="elementor-element elementor-element-4e8410c e-flex e-con-boxed e-con e-parent e-lazyloaded" data-settings="{&quot;background_background&quot;:&quot;gradient&quot;}">
                <div className="e-con-inner">
                  <div className="elementor-element elementor-element-2b792a40 elementor-widget-tablet__width-auto elementor-widget__width-initial elementor-widget elementor-widget-heading" data-widget_type="heading.default">
                    <div className="elementor-widget-container">
                      <h3 className="elementor-heading-title elementor-size-default" style={{"fontSize":"32px"}}>
                        We Accept Most Major Insurance
                      </h3>
                    </div>
                  </div>
                  <div className="elementor-element elementor-element-44f0cdd5 elementor-widget elementor-widget-text-editor" data-widget_type="text-editor.default">
                    <div className="elementor-widget-container">
                      <p style={{"fontSize":"16px"}}>
                        View our wide selection of accepted providers. Don’t see yours? Call our admissions team for help.
                      </p>
                    </div>
                  </div>
                  <div className="elementor-element elementor-element-3dc1f1f1 e-con-full e-grid e-con e-child">
                    <div className="elementor-element elementor-element-40e02466 elementor-widget__width-inherit elementor-widget elementor-widget-image" data-widget_type="image.default">
                      <div className="elementor-widget-container">
                        <Image src="/images/6a86ea6225c185032688ebc363011ca6.webp" alt="" width={964} height={462} className="attachment-full size-full wp-image-54427 entered exited" />
                      </div>
                    </div>
                    <div className="elementor-element elementor-element-3d19f025 elementor-widget__width-inherit elementor-widget elementor-widget-image" data-widget_type="image.default">
                      <div className="elementor-widget-container">
                        <Image src="/images/b9df7a3e649671088ef849f955ec7ecf.webp" alt="" width={965} height={462} className="attachment-full size-full wp-image-54428 entered exited" />
                      </div>
                    </div>
                    <div className="elementor-element elementor-element-420e7cc1 elementor-widget__width-inherit elementor-widget elementor-widget-image" data-widget_type="image.default">
                      <div className="elementor-widget-container">
                        <Image src="/images/b4d886afd6aa3d7a03d75c488a9fdb07.webp" alt="" width={964} height={462} className="attachment-full size-full wp-image-54429 entered exited" />
                      </div>
                    </div>
                    <div className="elementor-element elementor-element-33f54860 elementor-widget__width-inherit elementor-widget elementor-widget-image" data-widget_type="image.default">
                      <div className="elementor-widget-container">
                        <Image src="/images/eeec61988a0d9c842621c2b4c7900c87.webp" alt="" width={965} height={462} className="attachment-full size-full wp-image-54430 entered exited" />
                      </div>
                    </div>
                    <div className="elementor-element elementor-element-3e173f4a elementor-widget__width-inherit elementor-widget elementor-widget-image" data-widget_type="image.default">
                      <div className="elementor-widget-container">
                        <Image src="/images/837b0ee0c2a1de07e312cc2e12581182.webp" alt="" width={964} height={462} className="attachment-full size-full wp-image-54431 entered error" />
                      </div>
                    </div>
                    <div className="elementor-element elementor-element-64db1be3 elementor-widget__width-inherit elementor-hidden-desktop elementor-hidden-tablet elementor-widget elementor-widget-image" data-widget_type="image.default">
                      <div className="elementor-widget-container">
                        <Image src="/images/c0b07725778080957911708859362980.webp" alt="" width={580} height={319} className="attachment-full size-full wp-image-54432" />
                      </div>
                    </div>
                    <div className="elementor-element elementor-element-7ce3f99f elementor-widget__width-inherit elementor-hidden-mobile elementor-widget elementor-widget-image" data-widget_type="image.default">
                      <div className="elementor-widget-container">
                        <Image src="/images/5f24672b7ef463da8b4cfe323374408a.webp" alt="" width={965} height={462} className="attachment-full size-full wp-image-54433 entered error" />
                      </div>
                    </div>
                    <div className="elementor-element elementor-element-6f3dbcc1 elementor-widget__width-inherit elementor-hidden-desktop elementor-hidden-tablet elementor-widget elementor-widget-image" data-widget_type="image.default">
                      <div className="elementor-widget-container">
                        <Image src="/images/f0011968a01e8adbc23e8dcef59ef872.webp" alt="" width={580} height={319} className="attachment-full size-full wp-image-54434" />
                      </div>
                    </div>
                    <div className="elementor-element elementor-element-5a09283d elementor-widget__width-inherit elementor-hidden-mobile elementor-widget elementor-widget-image" data-widget_type="image.default">
                      <div className="elementor-widget-container">
                        <Image src="/images/388b13f9decfd0094e18335fe44db6d2.webp" alt="" width={964} height={462} className="attachment-full size-full wp-image-54435 entered error" />
                      </div>
                    </div>
                    <div className="elementor-element elementor-element-50cb78bf elementor-widget__width-inherit elementor-hidden-mobile elementor-widget elementor-widget-image" data-widget_type="image.default">
                      <div className="elementor-widget-container">
                        <Link href="/check-your-insurance/" style={{"fontSize":"16px"}}>
                          <Image src="/images/e9c3dac8b493ffbe58eae0def0ffd908.webp" alt="" width={580} height={319} className="attachment-full size-full wp-image-54436 entered error" />
                        </Link>
                      </div>
                    </div>
                    <div className="elementor-element elementor-element-6ddc9189 elementor-widget__width-inherit elementor-hidden-desktop elementor-hidden-tablet elementor-widget elementor-widget-image" data-widget_type="image.default">
                      <div className="elementor-widget-container">
                        <Link href="/check-your-insurance/" style={{"fontSize":"16px"}}>
                          <Image src="/images/e9c3dac8b493ffbe58eae0def0ffd908.webp" alt="" width={580} height={319} className="attachment-full size-full wp-image-54437" />
                        </Link>
                      </div>
                    </div>
                  </div>
                  <div className="elementor-element elementor-element-1367a6fe e-con-full e-flex e-con e-child">
                    <div className="elementor-element elementor-element-7477073a elementor-align-justify elementor-widget-mobile__width-inherit elementor-widget elementor-widget-button" data-widget_type="button.default">
                      <div className="elementor-widget-container">
                        <div className="elementor-button-wrapper">
                          <Link className="elementor-button elementor-button-link elementor-size-sm" href="tel:844-759-0999" style={{"fontSize":"16px"}}>
                            <span className="elementor-button-content-wrapper" style={{"fontSize":"16px"}}>
                              <span className="elementor-button-text" style={{"fontSize":"16px"}}>
                                844-759-0999
                              </span>
                            </span>
                          </Link>
                        </div>
                      </div>
                    </div>
                    <div className="elementor-element elementor-element-57d76186 elementor-align-justify elementor-widget-mobile__width-inherit elementor-widget elementor-widget-button" data-widget_type="button.default">
                      <div className="elementor-widget-container">
                        <div className="elementor-button-wrapper">
                          <Link className="elementor-button elementor-button-link elementor-size-sm" href="/check-your-insurance/" style={{"fontSize":"16px"}}>
                            <span className="elementor-button-content-wrapper" style={{"fontSize":"16px"}}>
                              <span className="elementor-button-text" style={{"fontSize":"16px"}}>
                                Verify Insurance
                              </span>
                            </span>
                          </Link>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
    <div className="elementor-element elementor-element-f4581b3 e-flex e-con-boxed e-con e-parent e-lazyloaded" data-settings="{&quot;background_background&quot;:&quot;gradient&quot;}">
      <div className="e-con-inner">
        <div className="elementor-element elementor-element-4adf6a6 elementor-widget elementor-widget-heading" data-widget_type="heading.default">
          <div className="elementor-widget-container">
            <h3 className="elementor-heading-title elementor-size-default" style={{"fontSize":"32px"}}>
              Related Pages
            </h3>
          </div>
        </div>
        <div className="elementor-element elementor-element-5b3948e elementor-widget elementor-widget-shortcode" data-widget_type="shortcode.default">
          <div className="elementor-widget-container">
            <div className="elementor-shortcode">
              <RelatedPages path={props.__path} />
            </div>
          </div>
        </div>
      </div>
    </div>
    <div className="elementor-element elementor-element-f9fa9a6 e-con-full e-flex e-con e-parent e-lazyloaded">
      <div className="elementor-element elementor-element-bad0189 elementor-widget elementor-widget-template" data-widget_type="template.default">
        <div className="elementor-widget-container">
          <div className="elementor-template">
            <div className="elementor elementor-54613 elementor-location-single">
              <div className="elementor-element elementor-element-d80816a e-flex e-con-boxed e-con e-parent e-lazyloaded" data-settings="{&quot;background_background&quot;:&quot;classic&quot;}">
                <div className="e-con-inner">
                  <div className="elementor-element elementor-element-357b19f e-con-full blog-last-sec e-flex e-con e-child">
                    <div className="elementor-element elementor-element-e4e2d01 elementor-widget elementor-widget-heading" data-widget_type="heading.default">
                      <div className="elementor-widget-container">
                        <h3 className="elementor-heading-title elementor-size-default" style={{"fontSize":"35px"}}>
                          You’re Not Alone.
                        </h3>
                      </div>
                    </div>
                    <div className="elementor-element elementor-element-d9e5ca1 elementor-widget__width-initial elementor-widget-mobile__width-initial elementor-widget-tablet__width-initial cta-para elementor-widget elementor-widget-text-editor" data-widget_type="text-editor.default">
                      <div className="elementor-widget-container">
                        <p style={{"fontSize":"17px"}}>
                          Get treatment from a team of expert staff who is passionate about helping you experience peace.
                        </p>
                      </div>
                    </div>
                    <div className="elementor-element elementor-element-7862528 elementor-align-left elementor-mobile-align-center elementor-tablet-align-left elementor-widget elementor-widget-button" data-widget_type="button.default">
                      <div className="elementor-widget-container">
                        <div className="elementor-button-wrapper">
                          <Link className="elementor-button elementor-button-link elementor-size-sm" href="tel:844-759-0999" style={{"fontSize":"16px"}}>
                            <span className="elementor-button-content-wrapper" style={{"fontSize":"16px"}}>
                              <span className="elementor-button-text" style={{"fontSize":"16px"}}>
                                Call Now
                              </span>
                            </span>
                          </Link>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
  <svg style={{"display":"none"}} className="e-font-icon-svg-symbols"></svg>
  <div className="asw-container">
    <div className="asw-widget">
      <a href="https://accessibility-widget.pages.dev" target="_blank" className="asw-menu-btn" title="Open Accessibility Menu" role="button" aria-expanded="false" aria-label="Open Accessibility Menu" style={{"--asw-bottom":"var(--asw-off-y)","--asw-left":"var(--asw-off-x)","--asw-off-x":"20px","--asw-off-y":"20px"}} rel="noopener noreferrer">
        <svg style={{"fill":"white"}} viewBox="0 0 24 24" width="30px" height="30px">
          <path d="M0 0h24v24H0V0z" fill="none"></path>
          <path d="M20.5 6c-2.61.7-5.67 1-8.5 1s-5.89-.3-8.5-1L3 8c1.86.5 4 .83 6 1v13h2v-6h2v6h2V9c2-.17 4.14-.5 6-1l-.5-2zM12 6c1.1 0 2-.9 2-2s-.9-2-2-2-2 .9-2 2 .9 2 2 2z"></path>
        </svg>
      </a>
    </div>
  </div>
  <svg style={{"display":"none"}} className="e-font-icon-svg-symbols"></svg>
</div>
    </>
  );
}
